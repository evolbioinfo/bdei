...
if __name__ == "__main__":
    ...
    for nwk, (_, row), out_nwk, out_log in zip(nwks, df.iterrows(), params.out_nwks, params.out_logs):
        ...
        n_tips = len(root)
        R0, inf_time, n_tips_declared, p = \
            row['R_naught'], row['infectious_time'], row['tree_size'], row['sampling_proba']
        assert n_tips_declared == n_tips
        with open(out_log, 'w+') as f:
            f.write('tips\tf\tu\tT\tla\tpsi\tp\n')
            psi = 1 / inf_time
            la = R0 * psi
            f.write('{tips}\t{f}\t{u}\t{T}\t{la}\t{psi}\t{p}\n'
                    .format(tips=n_tips, T=T, la=la, psi=psi, p=p, f=1, u=0))
        print('Tips={tips}\tT={T}\tla={la}\tpsi={psi}\tp={p}\n'
              .format(tips=n_tips, T=T, la=la, psi=psi, p=p))